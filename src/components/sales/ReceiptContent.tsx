...
export function ReceiptContent({
  ...
  const getPaymentBreakdown = () => {
    ...
    if ((sale.payment_type === 'single' || !sale.payment_type) && cash === 0 && card === 0 && bank === 0) {
      ...
      switch (String(sale.payment_method)) {
        case 'cash':
          payments.push({
            label: 'Pagato in Contanti:',
            amount: Number(sale.total_amount) || 0
          });
          break;
        case 'card':
          payments.push({
            label: 'Pagato con Carta:',
            amount: Number(sale.total_amount) || 0
          });
          break;
        case 'bank_transfer':
          payments.push({
            label: 'Pagato con Bonifico:',
            amount: Number(sale.total_amount) || 0
          });
          break;
        default:
  ...
  return <>
      ...
      fontSize: '10px'
        ...
        {payments.map((payment, index) => <div key={index} style={{
        display: 'flex',
