...
import { Search, Package, Smartphone, AlertTriangle, QrCode, Plus } from 'lucide-react';
...
export function ProductSearchSection() {
  ...
  const handleProductSelect = (product: Product) => {
    // Check availability
    const hasStock = product.has_serial ? product.product_units?.some(unit => unit.status === 'available') : product.stock > 0;
    if (!hasStock) {
      toast({
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          <div className="flex-1 relative">
            ...
            <Input value={searchTerm} onChange={e => handleSearchInput(e.target.value)} placeholder="Cerca per nome, IMEI o ultime 4 cifre..." className="pl-10" />
          </div>
        ...
        {/* Loading State */}
        {isLoading && <div className="text-center text-muted-foreground py-4">
            Ricerca in corso...
          </div>}

        {/* Search Results */}
        {searchResults.length > 0 && <div className="space-y-2 max-h-80 overflow-y-auto">
          ...
          return <div key={product.id} className="space-y-2">
                  {/* Product Header */}
                  <div className={`p-3 border rounded-lg ${!hasAvailableStock ? 'opacity-50' : 'cursor-pointer hover:bg-muted/50'} transition-colors`} onClick={() => !product.has_serial && hasAvailableStock && handleProductSelect(product)}>
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            <Badge variant={hasAvailableStock ? "default" : "destructive"}>
                              {product.has_serial ? `${availableUnits.length} unità` : `${product.stock} pz`}
                            </Badge>
                            {product.has_serial && <Badge variant="outline">
                    ...
                    return <div className="font-semibold">
                        ...
                        {!hasAvailableStock && <div className="text-xs text-destructive mt-1">
                            Non disponibile
                          </div>}
                      </div>
                    </div>
                  ...
                  {product.has_serial && availableUnits.length > 0 && <div className="ml-8 space-y-1">
                      {availableUnits.map(unit => <div key={unit.id} className="p-2 bg-muted/30 rounded border cursor-pointer hover:bg-muted/50 transition-colors" onClick={() => handleUnitSelect(product, unit)}>
                          <div className="flex items-center justify-between text-sm">
                            <div className="flex items-center gap-2">
                              <Badge variant="secondary" className="text-xs">
                                IMEI: {unit.serial_number}
                              </Badge>
                              {unit.barcode && <Badge variant="outline" className="text-xs">
                                  {unit.barcode}
                                </Badge>}
                            </div>
                            <div className="flex items-center gap-2">
                              {/* For salespersons, only show max selling price */}
                              {userRole === 'salesperson' ? <span className="text-xs font-semibold">
                                  €{(unit.max_price || product.max_price || product.price || 0).toFixed(2)}
                                </span> : unit.price && unit.price > 0 && <span className="text-xs font-semibold">
        ...
        {/* No Results */}
        {searchTerm.length >= 2 && !isLoading && searchResults.length === 0 && <div className="text-center text-muted-foreground py-4">
        ...
        {/* Search Instructions */}
        {!searchTerm && searchResults.length === 0 && <div className="text-center text-muted-foreground space-y-2 py-4">
