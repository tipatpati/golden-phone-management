...
export const SalesOverview = React.memo(() => {
  const { data: allSales = [], isLoading: salesLoading, error: salesError } = useSales();
  const { data: allRepairs = [], isLoading: repairsLoading } = useRepairs();
        ...
        <Card className="border-0 shadow-lg">
          ...
          <CardContent className="space-y-3 sm:space-y-4">
            {quickStats.map((stat, index) => (
              <div key={index} className="flex items-center justify-between p-2 sm:p-3 rounded-lg bg-gradient-to-r from-gray-50 to-gray-100">
