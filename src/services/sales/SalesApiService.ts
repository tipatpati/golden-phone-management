...
export class SalesApiService extends BaseApiService<Sale, CreateSaleData> {
  ...
  async create(saleData: CreateSaleData): Promise<Sale> {
    ...
    const saleNumber = await this.generateUniqueSaleNumber();
  ...
  private async generateUniqueSaleNumber(): Promise<string> {
    const now = new Date();
    const datePrefix = now.getFullYear().toString().slice(-2) + 
                       (now.getMonth() + 1).toString().padStart(2, '0') + 
                       now.getDate().toString().padStart(2, '0');
    
    // Get the latest sale number for today to determine the next sequence
    const { data: latestSale } = await this.supabase
      .from(this.tableName as any)
      .select('sale_number')
      .like('sale_number', `${datePrefix}%`)
      .order('sale_number', { ascending: false })
      .limit(1)
      .maybeSingle();
    
    let sequence = 1;
    if (latestSale && (latestSale as any).sale_number) {
      const lastSequence = parseInt((latestSale as any).sale_number.slice(-4));
