...
SET 
  min_price = CASE 
    WHEN min_price IS NULL OR min_price = 0 THEN 
      GREATEST(COALESCE(purchase_price, price, p.price) * 0.9, 1)
    ELSE min_price 
  END,
  max_price = CASE 
    WHEN max_price IS NULL OR max_price = 0 THEN 
      GREATEST(COALESCE(purchase_price, price, p.price) * 1.3, 1)
    ELSE max_price 
  END,
  updated_at = now()
FROM products p
...
INSERT INTO security_audit_log (
  ...
  auth.uid(),
  'data_sync_hybrid_pricing',
  jsonb_build_object(
    'operation', 'sync_existing_data_hybrid_pricing',
    'timestamp', now(),
    'description', 'Updated existing product units and transactions to match hybrid pricing model'
  ),
