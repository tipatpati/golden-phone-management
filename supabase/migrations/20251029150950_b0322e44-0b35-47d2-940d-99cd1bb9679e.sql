-- Fix create_sale_transaction to use get_product_effective_stock for consistent stock validation
-- This ensures the database function uses the same stock calculation as the frontend
...
BEGIN
  ...
  LOOP
    ...
    IF v_product_has_serial THEN
      ...
      END IF;
    ELSE
      -- For non-serialized products, use get_product_effective_stock for validation
      SELECT public.get_product_effective_stock((v_item->>'product_id')::uuid)
