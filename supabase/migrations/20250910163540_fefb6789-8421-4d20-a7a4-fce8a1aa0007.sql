...
-- First, let's see what triggers exist on the tables
SELECT schemaname, tablename, trigger_name, event_manipulation, action_statement 
FROM information_schema.triggers 
WHERE event_object_table IN ('products', 'product_units') 
...
-- 1. sync_product_stock_from_units() - Single source of truth for stock calculation
-- 2. Essential validation triggers
-- 3. History/audit triggers
...
BEGIN
  ...
  IF v_has_serial THEN
    ...
    -- Debug logging
    RAISE NOTICE '[DB TRIGGER] Updated product % stock to % (available units)', v_product_id, v_available_count;
  END IF;
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    IF NEW.serial_number IS NOT NULL AND NEW.serial_number <> '' THEN
      ...
      RAISE NOTICE '[DB TRIGGER] Marked unit % as sold', NEW.serial_number;
    END IF;
    RETURN NEW;
  END IF;
  ...
  IF TG_OP = 'DELETE' THEN
    IF OLD.serial_number IS NOT NULL AND OLD.serial_number <> '' THEN
      ...
      RAISE NOTICE '[DB TRIGGER] Restored unit % to available', OLD.serial_number;
    END IF;
    RETURN OLD;
  END IF;
...
-- Summary: We now have ONLY 2 triggers:
-- 1. sync_product_stock_trg on product_units (SINGLE SOURCE OF TRUTH for stock)
-- 2. update_unit_status_on_sale_trg on sale_items (ONLY updates unit status)