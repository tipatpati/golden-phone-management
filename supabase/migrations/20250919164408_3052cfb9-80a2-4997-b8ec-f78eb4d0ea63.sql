...
-- The issue is with unassigned record variables
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    IF NEW.serial_number IS NOT NULL AND btrim(NEW.serial_number) <> '' THEN
      ...
      -- Get sale data
      SELECT s.sale_number, s.payment_method, s.client_id, s.salesperson_id 
      INTO v_sale_number, v_payment_method, v_client_id, v_salesperson_id
      FROM public.sales s
